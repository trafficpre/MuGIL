...
from model_pems import MultiGraphInteraction
...
def main(save_results_flag, load_model_path, result_file):
    ...
    task_name = ['pems03', 'pems04', 'pems07', 'pems08']       # 'green',  'yellow', 'fhv'
    ...
    input_size = {'pems03': [12, 307], 'pems04': [12, 307], 'pems07': [12, 170], 'pems08': [12, 170]}
    ...
    class Encoder(nn.Module):
        def __init__(self):
            ...
            self.graph_act = MultiGraphInteraction(in_channels=12,
                                                   hiden_channels=[24, 122, 24, 122],
                                                   att_channels=[21, 66, 42, 65],
                                                   node_list=node_list,
                                                   task_list=['pems03', 'pems04', 'pems07', 'pems08'])
    ...
    # decoders = nn.ModuleDict({task: nn.Linear(163, 163*2) for task in list(task_dict.keys())})
    # decoders = nn.ModuleDict({'pemsd7': nn.Linear(228, 228), 'metr_la': nn.Linear(207, 207)})
    ...
    def visualize_result(h5_file, nodes_id, time_se, visualize_file, task):
        ...
        prediction = file_obj["predict_"+task][:]  # [N, T],切片，最后一维取第0列，所以变成二维了，要是[:, :, :1]那么维度不会缩减
        target = file_obj["target_"+task][:]  # [N, T],同上
        file_obj.close()

        plot_prediction = prediction[time_se[0]: time_se[1], nodes_id]  # [T1]，将指定节点的，指定时间的数据拿出来
        plot_target = target[time_se[0]: time_se[1], nodes_id]  # [T1]，同上
    ...
    task_vis = 'pems08'   # 'green', 'yellow', 'fhv'
    visualize_result(h5_file=result_file,
    # Node ID,     Time span [230,:] 凑出整天
    nodes_id = 143, time_se = [230, 230+288*3],  # 是节点的时间范围 24 * 12 * 2    node_id-34
    visualize_file = "green_node_2", task=task_vis)

if __name__=='__main__':
    save_results_flag = True
    # load_model_path = './results/best_adap+.pt'
    load_model_path = './results/best.pt'
    # result_file = './results/best.h5'
    # load_model_path = './result/seed=40/best.pt'
    result_file = './results/best.h5'
