...
# class MTformer(AbsArchitecture):
#     """Multi-gate Mixture-of-Experts (MMoE) for PEMS datasets.
#     """

#     def __init__(self, task_name, encoder_class, decoders, rep_grad, multi_input, device, **kwargs):
#         super(MTformer, self).__init__(task_name, encoder_class, decoders, rep_grad, multi_input, device, **kwargs)
...
class MTformer(AbsArchitecture):
    ...
    def forward(self, inputs, task_name=None):
        ...
        for i, task in enumerate(self.task_name):
            if task_name is not None and task != task_name:
                continue
            # out[task] = self.decoders[task](experts_shared_rep[i].transpose(1,2)).transpose(1,2)
